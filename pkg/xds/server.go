...
func (s *server) process(stream stream, reqCh <-chan *v2.DiscoveryRequest, defaultTypeURL string) error {
	...
	fmt.Printf("server.go: server.process(): start\n")
	fmt.Printf("server.go: server.process(): stream = %+v\n", stream)
	fmt.Printf("server.go: server.process(): defaultTypeURL = %+v\n", defaultTypeURL)
	...
	fmt.Printf("server.go: server.process(): watches = %+v\n", values)
	
	defer func() {
		fmt.Printf("server.go: server.process().fun(): deferred cancellation")
		values.Cancel()
		if s.callbacks != nil {
	...
	send := func(resp cache.Response, typeURL string) (string, error) {
		fmt.Printf("server.go: server.process().send(): resp = %+v\n", resp)
		...
		fmt.Printf("server.go: server.process().send(): out = %+v\n", out)
		fmt.Printf("server.go: server.process().send(): err = %+v\n", err)
		fmt.Printf("server.go: server.process().send(): resp = %+v\n", resp)
		fmt.Printf("server.go: server.process().send(): typeURL = %+v\n", typeURL)
	...
	fmt.Printf("server.go: server.process(): send() = %+v\n", send)
	
	// cdmitri:
	// we're are just initializing
	// the stream, and while request
	// exists from teh client, we might not
	// have any values for it in the cache.
	// However, there might be a static configuraiton
	// on the client that should be processessed without
	// being blocked. For this, send an empty response
	// to unblock the client if we are in the "default"
	// selection
	fmt.Printf("server.go: server.process(): send empty response\n")
	//server.go: server.process(): defaultTypeURL = type.googleapis.com/envoy.api.v2.ClusterLoadAssignment
	nonce, err := send(cache.Response{}, defaultTypeURL)
	fmt.Printf("server.go: server.process(): nonce = %+v\n", nonce)
	fmt.Printf("server.go: server.process(): err = %+v\n", err)
	
	for {
		fmt.Printf("server.go: server.process(): in for loop\n")
		select {
